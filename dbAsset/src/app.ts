...
export class AppController {
  ...
  private initMiddleware = (): void => {
    ...
    this.service.use(express.json({ limit: '50mb' }));
    ...
    this.service.use((req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      res.setHeader('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, x-auth, Authorization');
      next();
    });
  }
  ...
  private initControllers = (controllers: IRoutingController[]): void => {
    controllers.forEach((controller: IRoutingController) => {
      this.service.use('/api-service', controller.router);
    });
  }
  ...
  public listen() {
    this.service.listen(this.port, () => {
      logger.info(`Api Service started on PORT : [ ${this.port} ]`);
    });
  }
